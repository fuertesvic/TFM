...
for file in sorted(os.listdir('final_datasets_complete/')):
    
    print(f"\n\nProcessing {file}")    
    
    # Set the name for the columns and read file
    cols = ['Year','nodeID','SDcen','SClCoef','DBcen','DG','DI','DClCoef','DClcen','DKatz','Target']
    data = pd.read_csv(f"final_datasets_complete/{file}",sep=' ',names=cols)

    # Sort by ID
    sorted_data = data.sort_values(by=['nodeID','Year'])

    # Create Features to study and target values.
    features = ['nodeID','Year','SDcen','SClCoef','DBcen','DI','DG','DClCoef','DClcen','DKatz']
...
features_weight_df.to_csv(f"results/complete/FI_w", sep=',', index=False, encoding='utf-8')
features_gain_df.to_csv(f"results/complete/FI_g", sep=',', index=False, encoding='utf-8')
